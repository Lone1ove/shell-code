...
题目隔离机制：
- LangMem 工具使用 namespace=("shell_agent", "challenge", "{langgraph_user_id}")
...
def _competition_tools_enabled() -> bool:
    """
    是否启用比赛 API 工具。

    默认关闭，避免在纯渗透测试场景加载 submit_flag 等比赛工具。
    """
    return os.getenv("ENABLE_COMPETITION_TOOLS", "false").strip().lower() == "true"
...
def get_all_memory_tools(manual_mode: bool = False) -> List:
    ...
    Returns:
        所有工具的列表
    """
    from shell_agent.tools.memory_tools import get_memory_tools

    # 自定义记忆工具（题目隔离）
    memory_tools = get_memory_tools()

    # 手动模式或关闭比赛工具时，均不包含比赛 API 工具
    if manual_mode or not _competition_tools_enabled():
        log_system_event("[LangMem] 已跳过比赛 API 工具（submit_flag 等）")
        return [
            *memory_tools,
        ]

    # 兼容模式：包含比赛 API 工具
    from shell_agent.tools.competition_api_tools import get_competition_tools
