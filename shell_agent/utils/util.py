from shell_agent.common import log_state_update, log_system_event
from typing import List, Dict
import logging
import asyncio


def is_authentication_error(error: Exception | str) -> bool:
    """
    Check whether an exception/message is an authentication or authorization error.
    """
...
def is_context_overflow_error(error: Exception | str) -> bool:
    """
    Check whether an exception/message indicates LLM context/input is too large.
    """
    message = str(error).lower()
    patterns = (
        "error code: 413",
...
async def retry_llm_call(llm_func, *args, max_retries=5, base_delay=2.0, limiter=None, **kwargs):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Context overflow is deterministic; retrying with same payload won't help.
            if is_context_overflow_error(error_msg):
                log_system_event(
                    "[LLM错误] 上下文超限（413），停止重试",
                    {"error": error_msg},
                    level=logging.ERROR
                )
                raise
            ...
            is_retryable = any(pattern in normalized_error for pattern in retryable_patterns)
