...
from shell_agent.executor.base import BaseExecutor, ExecutionResult
...
def _repair_mojibake(text: str) -> str:
    """
    Best-effort fix for UTF-8 text that was mistakenly decoded as GB18030.
    Example: 将乱码文本修复为正常中文文本。
    """
    if not text:
        return text
    suspicious_markers = ("锛", "鏂", "鐩", "娴", "缁", "鎴", "鍙", "鉁", "鈿")
    ...
    try:
        ...
        return text
    old_score = sum(text.count(m) for m in suspicious_markers)
    new_score = sum(repaired.count(m) for m in suspicious_markers)
...
class DockerExecutor(BaseExecutor):
    ...
    def execute(self, command: str, timeout: int = 120, is_python: bool = False) -> ExecutionResult:
        ...
        try:
            ...
            stdout_str = _repair_mojibake(stdout_str)
            stderr_str = _repair_mojibake(stderr_str)
